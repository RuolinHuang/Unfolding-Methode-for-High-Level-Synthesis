...
public class Graph implements Iterable<Node> {
	...
	public Graph clone() {
		...
		for (Node nd : nodes.keySet()) {
			Node new_nd = new Node(nd.id, nd.getRT());	//copy node for each node
			for (Node su : nd.allSuccessors().keySet())
			{
				Node new_su = new Node(su.id, su.getRT());	//copy node for each successor
				newg.rlink(new_nd, new_su, nd.getSuccWeight(su));	//connect nodes with weight edge
			}
			newg.add(new_nd);
		}
		return newg;
